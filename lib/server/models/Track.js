...
var Track = new _Model2.default('track').field('name').defaultParam().required().string().done().field('duration').int().notIdentity().done().field('albumId').oneToOne().done().field('artistId').oneToOne().done().oneToMany(_File2.default, 'trackId').noDuplicates().done();
